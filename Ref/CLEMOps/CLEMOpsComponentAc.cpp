...
// \brief  cpp file for CLEMOps component base class
//
// \copyright
...
namespace Ref {
  ...
  namespace {
    ...
    typedef enum {
      CLEMOPS_COMPONENT_EXIT = Fw::ActiveComponentBase::ACTIVE_COMPONENT_EXIT,
      DATARESULT_DATARESULT,
      CMD_OPS_GET_DATA
    } MsgTypeEnum;
    ...
    typedef union {
      BYTE port1[Ref::InputDataResultPort::SERIALIZED_SIZE];
  ...
  Ref::InputDataResultPort *CLEMOpsComponentBase ::
    get_dataResult_InputPort(NATIVE_INT_TYPE portNum)
  {
    FW_ASSERT(portNum < this->getNum_dataResult_InputPorts(),static_cast<AssertArg>(portNum));
    return &this->m_dataResult_InputPort[portNum];
  }

  Fw::InputCmdPort *CLEMOpsComponentBase ::
  ...
  void CLEMOpsComponentBase ::
    set_dataRequest_OutputPort(
        NATIVE_INT_TYPE portNum,
        Ref::InputDataRequestPort* port
    )
  {
    FW_ASSERT(portNum < this->getNum_dataRequest_OutputPorts(),static_cast<AssertArg>(portNum));
    this->m_dataRequest_OutputPort[portNum].addCallPort(port);
  }
  ...
  {
    FW_ASSERT(portNum < this->getNum_dataRequest_OutputPorts(),static_cast<AssertArg>(portNum));
    return this->m_dataRequest_OutputPort[portNum].registerSerialPort(port);
  }
    ...
    regCommands(void)
  {
    ...
    this->m_CmdReg_OutputPort[0].invoke(
        this->getIdBase() + OPCODE_OPS_GET_DATA
    );
  }
  ...
  CLEMOpsComponentBase ::
    ...
    CLEMOpsComponentBase(const char* compName) :
    ...
    )
  {
    ...
    // Connect input port dataResult
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_dataResult_InputPorts();
        port++
    ) {

      this->m_dataResult_InputPort[port].init();
      this->m_dataResult_InputPort[port].addCallComp(
          this,
          m_p_dataResult_in
      );
      this->m_dataResult_InputPort[port].setPortNum(port);
      ...
      (void) snprintf(
          portName,
          sizeof(portName),
          "%s_dataResult_InputPort[%d]",
          this->m_objName,
          port
      );
      this->m_dataResult_InputPort[port].setObjName(portName);
#endif
    ...
    // Initialize output port dataRequest
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_dataRequest_OutputPorts();
        port++
    ) {
      this->m_dataRequest_OutputPort[port].init();
      ...
      (void) snprintf(
          portName,
          sizeof(portName),
          "%s_dataRequest_OutputPort[%d]",
          this->m_objName,
          port
      );
      this->m_dataRequest_OutputPort[port].setObjName(portName);
#endif
  ...
  {
    FW_ASSERT(portNum < this->getNum_dataRequest_OutputPorts(),static_cast<AssertArg>(portNum));
    this->m_dataRequest_OutputPort[portNum].invoke(data);
  }
  ...
  NATIVE_INT_TYPE CLEMOpsComponentBase ::
    getNum_dataRequest_OutputPorts(void)
  {
    return (NATIVE_INT_TYPE) FW_NUM_ARRAY_ELEMENTS(
        this->m_dataRequest_OutputPort
    );
  }

  NATIVE_INT_TYPE CLEMOpsComponentBase ::
    getNum_dataResult_InputPorts(void)
  {
    return (NATIVE_INT_TYPE) FW_NUM_ARRAY_ELEMENTS(
        this->m_dataResult_InputPort
    );
  }
  ...
  bool CLEMOpsComponentBase ::
    isConnected_dataRequest_OutputPort(NATIVE_INT_TYPE portNum)
  {
     FW_ASSERT(
        portNum < this->getNum_dataRequest_OutputPorts(),
        static_cast<AssertArg>(portNum)
     );
     return this->m_dataRequest_OutputPort[portNum].isConnected();
  }
    ...
    )
  {
    ...
    FW_ASSERT(callComp);
    CLEMOpsComponentBase* compPtr =
      (CLEMOpsComponentBase*) callComp;
    ...
    )
  {
    ...
    // Serialize for IPC
    _status = msg.serialize((NATIVE_INT_TYPE)CMD_OPS_GET_DATA);
    FW_ASSERT (
    ...
    tlmWrite_Ops_Result(F32 arg)
  {
    ...
    if (this->m_Tlm_OutputPort[0].isConnected()) {
      ...
      _id = this->getIdBase() + CHANNELID_OPS_RESULT;
    ...
    )
  {
    ...
    _id = this->getIdBase() + EVENTID_OPS_CMD_RECV;
    ...
    if (this->m_Log_OutputPort[0].isConnected()) {
      ...
      _status = _logBuff.serialize(
          static_cast<FwEnumStoreType>(dataEv)
      );
      FW_ASSERT(
          _status == Fw::FW_SERIALIZE_OK,
          static_cast<AssertArg>(_status)
    ...
    )
  {
    ...
    _id = this->getIdBase() + EVENTID_OPS_RESULT;
    ...
    )
  {
    ...
    CLEMOpsComponentBase* compPtr =
      (CLEMOpsComponentBase*) callComp;
    compPtr->dataResult_handlerBase(portNum, result);
  }
    ...
    )
  {
    ...
    _status = msg.serialize(
        static_cast<NATIVE_INT_TYPE>(DATARESULT_DATARESULT)
    );
    FW_ASSERT (
        _status == Fw::FW_SERIALIZE_OK,
        static_cast<AssertArg>(_status)
  ...
  Fw::QueuedComponentBase::MsgDispatchStatus CLEMOpsComponentBase ::
    doDispatch(void)
  {
    ComponentIpcSerializableBuffer msg;
    ...
    if (msgType == CLEMOPS_COMPONENT_EXIT) {
    ...
    switch (msgType) {
      ...
      // Handle async input port dataResult
      case DATARESULT_DATARESULT: {
